...
Materiał biologiczny stanowi krew obwodowa, z której izoluje się jednojądrzaste komórki krwi obwodowej (PBMC), zgodnie z układem doświadczalnym zastosowanym dla populacji referencyjnych [@fagny2015epigenomic]. Ze względu na to, że profil metylacji jest silnie zależny od typu komórki, równolegle rejestruje się skład subpopulacji komórkowych metodą cytometrii przepływowej z sortowaniem (FACS), co umożliwia późniejszą korektę heterogeniczności składu komórkowego. Wyizolowane DNA genomowe poddaje się kontroli ilości i integralności, a wymaganą masę wejściową dobiera się zgodnie z protokołem przygotowania bibliotek EM-seq, który pozwala na pracę z niewielkimi ilościami materiału [@vaisvila2021emseq].
...
Następnie koryguje się dwie kluczowe zmienne zakłócające: heterogeniczność składu komórkowego (na podstawie zarejestrowanych proporcji typów komórek krwi) oraz wiek osobnika, zgodnie z podejściem zastosowanym dla populacji referencyjnych [@fagny2015epigenomic].
...
<!-- 
UWAGA: ponieważ dane pochodzą z sekwencjonowania (zliczenia odczytów), a nie z macierzy (wartości beta), test różnicowy powinien zostać oparty na modelach zliczeniowych (`DSS`, `methylKit`, `dmrseq`) zamiast na modelu liniowym wartości beta/M z pakietu `limma`. 

TODO: poprawić `dms_contemporary.md` -->
